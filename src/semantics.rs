...
pub fn validate_xref_stream_dictionary(
    xref_stream_dict: &NameMap,
) -> Result<XRefStream, ValidationError> {
    ...
    match xref_stream_dict.get2(b"Length") {
        ...
        _ => return Err(ValidationError::NoGoodLengthInXRefStreamDictionary),
    }
    ...
    match xref_stream_dict.get2(b"Size") {
        ...
        _ => return Err(ValidationError::NoGoodSizeInXRefStreamDictionary),
    }
    ...
    match xref_stream_dict.get2(b"Filter") {
        ...
        Some(PdfObject::Array(vec_of_pdf_objects)) => {
            ...
            for filter in &vec_of_pdf_objects {
                match filter {
                    PdfObject::Name(name_value) => {
                        filter_vec.push(PdfObject::Name(name_value.clone()));
                    }
                    _ => {
                        return Err(ValidationError::NoGoodFilterInXRefStreamDictionary);
                    }
                }
            }
            ...
            match xref_stream_dict.get2(b"DecodeParams") {
                Some(PdfObject::Array(vec_of_param_dicts)) => {
                    ...
                    for decode_param in &vec_of_param_dicts {
                        match decode_param {
                            ...
                            _ => {
                                return Err(
                                    ValidationError::DecodeParamValueExpectedToBeDictionaryOrNull,
                                );
                            }
                        }
                    }
                ...
                None => {}
                _ => {
                    return Err(ValidationError::DecodeParamsExpectedToBeArrayOfDictionaryOrAbsent)
                }
            }
        }
        Some(PdfObject::Name(name_value)) => {
            ...
            match xref_stream_dict.get2(b"DecodeParams") {
                Some(PdfObject::Dictionary(decode_params_map)) => {
                    result.decode_params =
                        Some(vec![PdfObject::Dictionary(decode_params_map.clone())]);
                }
                None => {}
        ...
        Some(PdfObject::Array(vec_of_possibly_integers)) => {
            ...
            for chunk in vec_of_possibly_integers.chunks(2) {
                match chunk {
                    ...
                    _ => return Err(ValidationError::IndexElementsOtherThanPositiveIntegers),
                }
            }
    ...
    match xref_stream_dict.get2(b"W") {
        Some(PdfObject::Array(vec_of_possibly_3_integers)) => {
            ...
            let w_0 = match vec_of_possibly_3_integers[0] {
                PdfObject::Integer(v0) if v0 >= 0 => v0 as usize,
    ...
    match xref_stream_dict.get2(b"Prev") {
        ...
        None => {}
        _ => return Err(ValidationError::NoGoodPrevInXRefStreamDictionary),
    }
